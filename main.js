...
function saveGame() {
    ...
    localStorage.setItem('fantasyIdleSave', JSON.stringify(state));
}

function loadGame() {
    const saved = localStorage.getItem('fantasyIdleSave');
    if (saved) {
        try {
...
function spawnEnemy() {
    ...
    state.combat.enemy = {
        name: isBoss ? `Boss Stage ${stage}` : `Monster Lv ${stage}`,
        hp: isBoss ? hp * 5 : hp,
...
function generateProceduralEquipment(category, targetType, barId) {
    ...
    return {
        id: ++state.idCounter,
        name: `[${rarity.name}] ${material.name} ${TYPE_NAMES[targetType]}`,
        type: targetType,
        atk: baseAtk,
        def: baseDef,
...
function generateProceduralJewelry(targetType, barId, gemId) {
    ...
    return {
        id: ++state.idCounter,
        name: `[${rarity.name}] ${gem.name} ${TYPE_NAMES[targetType]}`,
        type: targetType,
        atk: baseAtk,
        def: baseDef,
        critChance, critDmg, dodgeChance, speedBonus,
...
function updateCombatUI() {
    ...
    if (stEl) {
        ...
        if (gearContainer) {
            let gearHtml = `<div style="display:flex; gap:0.5rem; flex-wrap:wrap; font-size:0.75rem; justify-content:center;">`;
            for (const slot in state.equipped) {
                if (state.equipped[slot]) {
                    gearHtml += `<span style="background:rgba(255,255,255,0.05); padding:2px 6px; border-radius:4px; color:${state.equipped[slot].color || '#e2e8f0'}">${state.equipped[slot].name}</span>`;
                }
            }
            if (gearHtml === `<div style="display:flex; gap:0.5rem; flex-wrap:wrap; font-size:0.75rem; justify-content:center;">`) {
                    ...
                    <div class="inv-item" style="border-color: ${item.color}40; background: linear-gradient(to right, ${item.color}10, transparent);">
                        ...
                        <div class="inv-stats-row">
                            <div class="item-stats" style="margin: 0; gap: 0.5rem; flex-direction: column;">
                                ...
                                ${!item.atk && !item.def && !sStats ? '<span style="color:var(--text-muted)">No combat</span>' : ''}
                            </div>
